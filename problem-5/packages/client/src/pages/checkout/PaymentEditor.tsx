...
const CryptoEditorTab = React.lazy(() => import('./CryptoEditorTab'))
    ...
    <TabContext value={tabState}>
      <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
        <TabList
          ...
          <Tab label='Crypto Wallet' value='crypto' />
        </TabList>
      </Box>
      <TabPanel value='card'>
        <Suspense
      ...
      <TabPanel value='crypto'>
        <Suspense
          fallback={
            <Typography variant='body1'>Loading crypto editor...</Typography>
          }>
          <CryptoEditorTab
            crypto={checkout.crypto}
            dispatch={dispatch}
            setAddressTab={setAddressTab}
            setFactorSalesTax={setFactorSalesTax}
            setShowBillingAddress={setShowBillingAddress}
